...
Справка по CLI: python3 multiprocessing_pool_template.py -h
...
class PrepSingleProc():
        ...
        берущая на себя всю работу
        над каждым файлом, должна
        подаваться в map, а значит,
        принимать только имя файла
        из скармливаемого этому
        map списка таких имён.
        Как тогда передать функции
        любые другие аргументы?
        Выход из положения - создать
        класс, насытить его нужными
        атрибутами, заодно протащив
        их в функцию посредством self.
        '''
        
        def __init__(self, args):
                '''
                Поскольку эта программа - всего
                лишь шаблон для написания серьёзных
                решений, набор атрибутов ограничен
                путями к исходной и конечной папкам.
                '''
...
argparser.add_argument('-t', '--trg-dir-path', metavar='путь', dest='trg_dir_path', type=str,
                       help='Путь к папке для конечных файлов')
...
#функцию обработки одного исходного файла.
...
#Программа не допустит,
#чтобы процессов было больше,
#чем обрабатываемых файлов.
#Принимаемая мера при таком
#раскладе - приравнивание первого
#из перечисленных ко второму.
#Иначе это будет от слова
#бессмысленно до небезопасно.
#Также программа вынуждена пресечь
#душевный порыв исследователя
#задать как можно большее
#количество процессов:).
#При работе с громадным числом
#файлов на овердофигаядерном
#сервере, безусловно, хочется зверски
#распараллелить выполнение задачи.
#Почему прога такая злая и не даёт
#большого простора в этой области?
...
print(f'\nКоличество параллельно обрабатываемых файлов: {proc_quan}')
